...
class FormController extends Controller
{
    ...
    public function applicationYear()
    {
        $years = FormModel::select('application_year')
            ->where('is_deleted', 0)
            ->distinct()
            ->orderBy('application_year', 'desc')
            ->pluck('application_year');

        return view('applications.application_year', compact('years'));
    }
    ...
    public function filterByYear(Request $request)
    {
        ...
        $year = $request->application_year;

        $applications = FormModel::with('country', 'programme')
